...
impl InputFieldWidget {
    ...
    fn get_block(&self, is_focused: bool) -> Block<'_> {
        ...
        let block = if self.style.show_border {
            // Use plain ASCII border type for Terminus font compatibility
            block
                .borders(Borders::ALL)
    ...
    pub fn render(
        ...
        // If borders are hidden (Terminus mode), add manual ASCII styling
        if !self.style.show_border {
            if self.style.show_title {
                let title_style = if is_focused {
                    Style::default().fg(get_color(&self.style.title_color_focused))
                } else {
                    Style::default().fg(get_color(&self.style.title_color))
                };
                let title_widget = Paragraph::new(self.style.title.clone()).style(title_style);
            ...
            if inner.width > 2 {
                ...
                let left_bracket = Paragraph::new("[").style(bracket_style);
                frame.render_widget(left_bracket, Rect::new(area.x, inner.y, 1, 1));

                let right_bracket = Paragraph::new("]").style(bracket_style);
                frame.render_widget(right_bracket, Rect::new(inner.x + inner.width, inner.y, 1, 1));
            }
        }
        ...
        let widget = Paragraph::new(show_string).style(self.get_text_style(is_focused));
