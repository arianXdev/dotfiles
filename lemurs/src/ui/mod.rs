...
use footer_deco::FooterDecoWidget;
...
use self::welcome_header::WelcomeHeaderWidget;
...
pub struct LoginForm {
    ...
    /// When the login process started (for animations)
    login_start: Arc<Mutex<Option<Instant>>>,
}
...
impl LoginForm {
    ...
    pub fn new(config: Config, preview: bool) -> LoginForm {
        LoginForm {
            ...
            config,
            network_status: Arc::new(Mutex::new(NetworkStatus::default())),
            login_start: Arc::new(Mutex::new(None)),
        }
    }
    ...
    pub fn run(self, terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) -> io::Result<()> {
        ...
        let login_start_render = self.login_start.clone();

        // Spawn background network thread
        let network_update_status = self.network_status.clone();
        std::thread::spawn(move || loop {
            std::thread::sleep(Duration::from_secs(1));
            if let Ok(mut status) = network_update_status.lock() {
                status.probe();
            }
        });
        ...
        std::thread::spawn(move || {
            ...
            // Dedicated UI Heartbeat: Ensures animations (Matrix typing, uptime) continue
            // even if the main event thread is blocked by authentication or delays.
            let ticker_send = req_send_channel.clone();
            std::thread::spawn(move || loop {
                std::thread::sleep(Duration::from_millis(50));
                let _ = ticker_send.send(UIThreadRequest::Redraw);
            });
            ...
            let pre_environment = || {
                ...
                if let Ok(mut start) = self.login_start.lock() {
                    *start = Some(Instant::now());
                }

                send_ui_request(UIThreadRequest::Redraw);

                // Delay to allow the user to see the "Welcome back" greeting
                if self.config.login_delay_ms > 0 {
                    std::thread::sleep(Duration::from_millis(self.config.login_delay_ms));
                }
        ...
        while let Ok(request) = req_recv_channel.recv() {
            match request {
                UIThreadRequest::Redraw => {
                    let draw_action = terminal.draw(|f| {
                        let layout = Chunks::new(f);
                        let elapsed_ms = if let Ok(start_opt) = login_start_render.lock() {
...
fn login_form_render<B: Backend>(
    ...
    background: BackgroundWidget,
    key_menu: KeyMenuWidget,
    _environment: Arc<Mutex<SwitcherWidget<PostLoginEnvironment>>>,
    username: Arc<Mutex<InputFieldWidget>>,
    password: Arc<Mutex<InputFieldWidget>>,
    ...
    let network = if let Ok(n) = network_status.lock() {
        n.clone()
    } else {
    ...
    key_menu.render(frame, chunks.key_menu, status_message.clone());
    
    // Render sci-fi welcome header
    let welcome_header = WelcomeHeaderWidget::new();
    welcome_header.render(frame, chunks.welcome_header, network.clone(), status_message.clone());
    ...
    StatusMessage::render(status_message.clone(), frame, chunks.status_message);

    // Render sci-fi footer decorations
    let footer_deco = FooterDecoWidget::new();
    footer_deco.render(frame, chunks.footer_deco, &network, status_message, elapsed_ms);
}
